...
contract('Administrated', async (accounts) => {
 ...
 it('should not set admin if not owner', async () => {
   let instance = await Administrated.new();
   await instance.setAdmin(accounts[1], true, { from: accounts[1] }).should.be.rejectedWith(testUtils.REQUIRE_FAIL);
 });
 ...
 it('should transfer ownership', async () => {
   ...
   await instance.setAdmin(accounts[0], true, { from: accounts[0] }).should.be.rejectedWith(testUtils.REQUIRE_FAIL);
   await instance.setAdmin(accounts[1], true, { from: accounts[1] });
 ...
 it('should set admin if not owner and admin', async () => {
   let instance = await Administrated.new();
   await instance.setAdmin(accounts[2], true, { from: accounts[1] }).should.be.rejectedWith(testUtils.REQUIRE_FAIL);
   await instance.setAdmin(accounts[1], true);
   await instance.setAdmin(accounts[2], true, { from: accounts[1] });
