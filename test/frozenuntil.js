...
contract('FrozenUntil', async (accounts) => {
  ...
  it('should not allow function call with modifer', async () => {
    ...
    let isFrozen = await instance.isFrozen(now);
    expect(isFrozen).to.be.true;
    await instance.testNotFrozen().should.be.rejectedWith(testUtils.REQUIRE_FAIL);
    await instance.testFrozen().should.not.be.rejectedWith(testUtils.REQUIRE_FAIL);
    await instance.freezeUntil(0);
    isFrozen = await instance.isFrozen(now);
    expect(isFrozen).to.be.false;
    await instance.testNotFrozen().should.not.be.rejectedWith(testUtils.REQUIRE_FAIL);
    await instance.testFrozen().should.be.rejectedWith(testUtils.REQUIRE_FAIL);
  });
